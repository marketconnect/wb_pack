...
func main() {
	...
	for _, file := range files {
		...
		name, transportVolume, lengthMM, widthMM := getProductData(db, productID)
		if name == "" {
...
func getProductData(db *sql.DB, productID string) (string, string, string, string) {
	var name, transportVolume, lengthMM, widthMM string
	err := db.QueryRow("SELECT name, transport_volume_m3, length_mm, width_mm FROM products WHERE product_id = ?", productID).
		Scan(&name, &transportVolume, &lengthMM, &widthMM)
	if err != nil {
	...
	return name, transportVolume, lengthMM, widthMM
}
